...
type
   ...
   public
      ...
      [ForeignKey('FK1_TSTOPRODUCT', 'COMPANYID', 'TMNGCOMPANY', 'CODE',
        SetNull, SetNull)]
      ...
      [Dictionary('CODE_SINAPI', 'Mensagem de validação', '', '', '',
        taLeftJustify)]
      ...
      [Dictionary('UNITMEASURE', 'Mensagem de validação', '', '', '',
        taLeftJustify)]
      property UNITMEASURE: String read FUNITMEASURE write FUNITMEASURE;

      [Restrictions([NotNull])]
      [Column('ORIGINPRICE', ftString, 10)]
      [Dictionary('ORIGINPRICE', 'Mensagem de validação', '', '', '',
        taLeftJustify)]
      property ORIGINPRICE: String read FORIGINPRICE write FORIGINPRICE;

      [Column('AVERAGEPRICE', ftBCD, 18, 4)]
      [Dictionary('AVERAGEPRICE', 'Mensagem de validação', '0', '', '',
        taRightJustify)]
      property AVERAGEPRICE: nullable<Double> read FAVERAGEPRICE
        write FAVERAGEPRICE;

      [Restrictions([NotNull])]
      [Column('AVERAGEPRICE_SINAPI', ftBCD, 18, 4)]
      [Dictionary('AVERAGEPRICE_SINAPI', 'Mensagem de validação', '0', '', '',
        taRightJustify)]
      property AVERAGEPRICE_SINAPI: Double read FAVERAGEPRICE_SINAPI
        write FAVERAGEPRICE_SINAPI;
      ...
      [ForeignKey('FK2_TSTOPRODUCT', 'USERID', 'TMNGUSER', 'CODE',
        SetNull, SetNull)]
