...
type
   ...
   public
      ...
      function userNameExists(AUserCode: string; AUserName: string)
        : Boolean; overload;

      function emailExists(AEmail: string): Boolean; overload;
      function emailExists(AUserCode: string; AEmail: string): Boolean;
        overload;
...
begin
   try
      ...
      try
         ...
         Result := BCrypt.CheckPassword(APassword, FUserAuthenticated.keypass,
           hashNeedUpgrade);
      finally
         BCrypt.Free;
      end;
...
begin
   ...
   if FUserModel.DAO.FindWhere(Format('CODE <> %s and USERNAME = %s',
     [QuotedStr(AUserCode), QuotedStr(AUserName)])).Count > 0 then
      Result := True;
end;
...
begin
   ...
   if FUserModel.DAO.FindWhere(Format('USERNAME = %s', [QuotedStr(AUserName)]))
     .Count > 0 then
      Result := True;
end;
...
begin
   FUserModel := TFacadeModel.New.moduleFacade.managmentFactoryModel.userModel;
end;
...
begin
   ...
   if FUserModel.DAO.FindWhere(Format('EMAIL = %s', [QuotedStr(AEmail)]))
     .Count > 0 then
      Result := True;
end;

function TUserController.emailExists(AUserCode: string; AEmail: string)
  : Boolean;
begin
   Result := False;

   if FUserModel.DAO.FindWhere(Format('CODE <> %s and EMAIL = %s',
     [QuotedStr(AUserCode), QuotedStr(AEmail)])).Count > 0 then
      Result := True;
end;
