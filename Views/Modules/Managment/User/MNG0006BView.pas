...
uses
   ...
   Base.View.interf,
   Types.Controllers, cxControls, cxContainer, cxEdit, cxMaskEdit,
...
begin
   { 1 } if userMailExists then
      Exit;
   { 2 } if userNameExists then
      Exit;
...
begin
   FUserController.duplicate.name(TxName.Text).email(TxEmail.Text)
     .username(TxUserName.Text).keypass(TxKeyPass.Text)
     .status(CbStatus.ItemIndex).save
end;
...
begin
   FUserController.insert.name(TxName.Text).email(TxEmail.Text)
     .username(TxUserName.Text).keypass(TxKeyPass.Text)
     .status(CbStatus.ItemIndex).save
end;
...
begin
   TFacadeView.New.messagesFactoryView.typeMessage(tmWarning)
...
begin
   TFacadeView.New.messagesFactoryView.typeMessage(tmWarning)
     .Messages(Format('Já existe um usuário "%s" ', [TxUserName.Text])).&end;
end;
...
begin
   FUserController.update.name(TxName.Text).email(TxEmail.Text)
     .username(TxUserName.Text).keypass(TxKeyPass.Text)
     .status(CbStatus.ItemIndex).save
end;
...
begin
   ...
   case FOperation of
      ...
      toUpdate:
         begin
            if FUserController.emailExists(FUserController.code, TxEmail.Text)
            then
            begin
               showMessageDuplicatedMail;
...
begin
   ...
   case FOperation of
      ...
      toUpdate:
         begin
            if FUserController.userNameExists(FUserController.code,
              TxUserName.Text) then
            begin
               showMessageDuplicatedUserName;
