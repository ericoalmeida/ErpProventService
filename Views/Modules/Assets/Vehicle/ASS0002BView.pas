...
uses
   ...
   cxContainer, cxEdit,
   cxLabel, cxTextEdit, cxMaskEdit, cxDropDownEdit, Base.View.interf,
   Types.Controllers,
...
begin
   FVehicleController.delete.save;
end;
...
begin
   FVehicleController.duplicate.companyId(FSessionCompany)
     .description(TxDescription.Text).model(TxModel.Text).brand(TxBrand.Text)
     .category(TxCategory.Text).board(TxBoard.Text).status(CbStatus.ItemIndex)
...
begin
   FVehicleController.insert.companyId(FSessionCompany)
     .description(TxDescription.Text).model(TxModel.Text).brand(TxBrand.Text)
     .category(TxCategory.Text).board(TxBoard.Text).status(CbStatus.ItemIndex)
     .userId(FSessionUser).save;
end;
...
begin
   FVehicleController.update.companyId(FSessionCompany)
     .description(TxDescription.Text).model(TxModel.Text).brand(TxBrand.Text)
     .category(TxCategory.Text).board(TxBoard.Text).status(CbStatus.ItemIndex)
     .userId(FSessionUser).save;
end;
